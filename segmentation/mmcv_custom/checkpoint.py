...
def load_checkpoint(model,
    ...
    if 'state_dict' in checkpoint:
        ...
        state_dict = checkpoint['student'] # for dino
    elif 'target_encoder' in checkpoint:
        state_dict = checkpoint['target_encoder']
        print("we load checkpoints here")
    else:
    ...
    if 'pos_embed' in state_dict:  #and model.use_abs_pos_emb:
        ...
        has_cls_pos_embeding = True
        if orig_size * orig_size != pos_embed_checkpoint.shape[-2] - num_extra_tokens:
            has_cls_pos_embeding = False
            orig_size = int(pos_embed_checkpoint.shape[-2] ** 0.5)
            assert orig_size ** 2 == pos_embed_checkpoint.shape[-2]
        ...
        if orig_size != new_size:
            ...
            if extra_tokens is not None:
                ...
                new_pos_embed = torch.cat((model.pos_embed[:,0:1,:], pos_tokens), dim=1)
            
            state_dict['pos_embed'] = new_pos_embed
        # remove the pe of dist token
        if num_extra_tokens == 2:
            print('removing the pe of dist token')
            state_dict['pos_embed'] = torch.cat((pos_embed_checkpoint[0:1], pos_embed_checkpoint[2:]), dim=1)

    if 'cae.pos_embed' in state_dict:  #and model.use_abs_pos_emb:
        pos_embed_checkpoint = state_dict['cae.pos_embed']
        embedding_size = pos_embed_checkpoint.shape[-1]
        num_patches = model.cae.patch_embed.num_patches
        num_extra_tokens = model.cae.pos_embed.shape[-2] - num_patches
        # height (== width) for the checkpoint position embedding
        orig_size = int((pos_embed_checkpoint.shape[-2] - num_extra_tokens) ** 0.5)
        # height (== width) for the new position embedding
        new_size = int(num_patches ** 0.5)
        ...
        if orig_size != new_size:
            ...
            pos_tokens = pos_tokens.permute(0, 2, 3, 1).flatten(1, 2)
            new_pos_embed = torch.cat((extra_tokens, pos_tokens), dim=1)
            state_dict['cae.pos_embed'] = new_pos_embed
    ...
    all_keys = list(state_dict.keys())
    # print("all keys:", len(state_dict), state_dict.keys())

    # random init the last several stages
    if random_init_stages >= 0:
        for key in all_keys:
            if key.startswith('blocks.') and int(key[7:].split('.')[0]) >= random_init_stages:
                state_dict.pop(key)
